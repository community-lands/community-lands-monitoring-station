...
function fetch(dir, src_url, target_file, cb){
  queueable = function(){
    ...
    mkdirp(dir, function (err) {
      ...
      var request = http.get(src_url, function(response) {
        response.pipe(file);
...
module.exports.pyramid_urls = function(from_lon, from_lat, to_lon, to_lat, start_zoom, end_zoom){
  tiles_needed = 0;
  urls = [];
  for(zoom=start_zoom;zoom<=end_zoom;zoom++){ // # Bing maps max zoom is 19
    console.log("Calculating tiles for zoom level " + zoom);
    from_y_tile = lat2tile(from_lat,zoom);
    to_y_tile = lat2tile(to_lat,zoom);
    if(to_y_tile<from_y_tile){
      tmp = from_y_tile;
      from_y_tile = to_y_tile;
      to_y_tile = tmp;
    }
    from_x_tile = lon2tile(from_lon, zoom);
    to_x_tile = lon2tile(to_lon, zoom);
    ...
    for(xx=from_x_tile;xx<=to_x_tile;xx++){
      for(yy=from_y_tile;yy<=to_y_tile;yy++){
        // src_url = "http://a.tile.openstreetmap.org/"+zoom+"/"+xx+"/"+yy+".png"
        src_url = "http://a.tile.openstreetmap.fr/hot/"+zoom+"/"+xx+"/"+yy+".png"
        // This doesn't work, Google will throttle
        // src_url = "http://mt0.google.com/vt/lyrs=s@169000000&hl=en&x="+xx+"&y="+yy+"&z="+zoom+"&s=Ga"
        target_file = "./Monitoring/Maps/Tiles/"+zoom+"/"+xx+"/"+yy+".png"
        dir = "./Monitoring/Maps/Tiles/"+zoom+"/"+xx
        fetch(dir, src_url, target_file, function (err){
          if (err) console.error(err)
        });
      }
    }
    count = Math.abs(to_y_tile - from_y_tile + 1) * Math.abs(to_x_tile - from_x_tile + 1)
