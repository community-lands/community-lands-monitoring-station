...
var buildName = config.APP_NAME.replace(/\s/g, '_') + '_v' + config.APP_VERSION
var dmgPath = path.join(distFolder, buildName + '_macOS.dmg')

function createConfiguration () {
  var installerFolder = path.join(distFolder, config.APP_FILE_NAME + '-darwin-x64')
  ...
  return {
    ...
    specification: {
      ...
      icon: path.join(config.APP_ICON_PATH, 'community-lands.icns'),
      'icon-size': 128,
      contents: [
